...
import skylands.logic.util.ChunkScanQueue;
import skylands.util.FontUtils;
import skylands.util.Texts;
import skylands.util.UnitConvertions;
...
public class LevelCommand {
    ...
    public static void init(CommandDispatcher<ServerCommandSource> dispatcher) {
        register(dispatcher, node()
            .then(literal("level")
                .requires(requiresIsland("skylands.command.level", true))
                .then(literal("scan")
                    .executes(context -> LevelCommand.run(context.getSource()))
                )
            )
        );
    }
    ...
    private static int run(ServerCommandSource source) {
        ...
        if (island.isScanning()) {
            source.sendError(Texts.of("message.skylands.island.level.scan.running"));
            return 0;
        }
        ...
        MutableText text = Texts.of("message.skylands.island.level.scan.background").copy();

        Text startScanText = Texts.of("message.skylands.island.level.scan.start");
        Text cancelText = Texts.of("message.skylands.island.level.scan.cancel");
        ...
        VirtualElement.InteractionHandler startScan = createHandler((interactor, hand) -> {
            ...
            scanChunks(island, () -> {
                ...
                display.setText(Texts.of("message.skylands.island.level.scan.preparing"));
            }, total -> {
                if (total > 200) {
                    display.setText(Texts.of("message.skylands.island.level.scan.progress", new MapBuilder.StringMap()
                        .putAny("progress", 0)
                        .putAny("total", total)
                        .build()));
                    return;
                ...
                if (total > 200) {
                    Text progress = Texts.of("message.skylands.island.level.scan.progress", new MapBuilder.StringMap()
                        .putAny("progress", current)
                        .putAny("total", total)
                        .build());
                    Skylands.getInstance().scheduler.scheduleDelayed(new Identifier("skylands", "island-scan/" + island.getIslandId().toString()), Skylands.getServer(), 0, () -> display.setText(progress));
                    return;
                }
                ...
                Skylands.getInstance().scheduler.scheduleDelayed(new Identifier("skylands", "island-scan/" + island.getIslandId().toString()), Skylands.getServer(), 0, () -> display.setText(visualization));
            }, (timeTaken, scannedBlocks) -> {
                Skylands.getInstance().scheduler.scheduleDelayed(Skylands.getServer(), 0, () -> {
                    source.sendFeedback(Texts.of("message.skylands.island.level.scan.time_taken", new MapBuilder.StringMap()
                    ...
                    scannedBlocks.forEach((block, amount) -> {
                        if (i.get() > 8) return;
                        ...
                        Text amountText = Texts.of(amount + "x " + block.getName().getString());
                    ...
                    ref.closeButton = createInteraction(Texts.of("message.skylands.island.level.scan.close"), createHandler((player1, hand1) -> {
        ...
        Skylands.getInstance().scheduler.scheduleDelayed(Skylands.getServer(), 15, () -> {
            message.set(createDisplay(Texts.of("message.skylands.island.level.scan.confirm"), yaw, new Vec3d(0, 0.25 * (lines / 2d) + 0.0625 + 0.5, 0)));
        ...
        source.sendFeedback(Texts.of("message.skylands.island.level.scan.opening"), false);
        return 1;
    }
        ...
        Thread scanThread = new Thread(() -> {
            ...
            if (radius <= 200) {
                ...
                chunkHolders.forEach(chunkHolder -> {
                    chunks.add(chunkHolder.getCurrentChunk());
                });
            }
            ...
            while (!chunks.finished()) {
                ...
                for (ChunkSection section : nonEmptySections) {
                    ...
                    stateContainer.count((blockState, amount) -> {
                        ...
                        blocks.compute(blockState.getBlock(), (state, count) -> count == null ? amount : count + amount);
                    });
                }
            }
            ...
            List<Map.Entry<Block, Integer>> entries = new LinkedList<>(blocks.entrySet());
    ...
    interface ScanFinish {
        void finish(long timeTaken, Map<Block, Integer> scannedBlocks);
    }
}
